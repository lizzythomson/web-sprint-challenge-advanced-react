...
The main difference between a class-based component and a functional component is in the name. The class-based components uses classes whereas the functional component uses functions instead. One difference when writing code is how we refer to state. Class-based components use this.state to handle state and functional components use the useState hook. Another difference in writing the code is how we pass props. Class-based components use the "this" keyword to refer to props and functional components pass props as an argument. Another difference between the two is the lifecycle methods are handled in useEffect in functional components whereas componentDidMount is an example of a lifecyle method used in class-based components.

2. When does a componentWillMount function be called? What about a componentDidUpdate?

The componentWillMount function will be called before the render method code is run in the browser. The componentWillMount is part of the mounting phase.

(However, I think that componentWillMount is now considered legacy code and typically programmers now use componentDidMount - which mounts right after the first render is completed.)

The componentDidUpdate function will be called right after a component is updated(for example new props from a parent will trigger an update to the child). The componentDidUpdate is part of the updating phase.

3. Define stateful logic.

Stateful logic is logic that uses state and manipulates/creates that data.

4. What are the three step of creating a successful test? What is done in each phase?

Arrange - verifies the setup of the react component you are testing (ex. render(</App />))
Act - executes behavior (ex. typing into an input, clicking a button)
Assert - gets the reponse and verifies that the reponse is what we expect it to be (ex. we expect some text to be rendered on a page)
